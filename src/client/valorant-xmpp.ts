...
const buildConfig = (options: ValorantXmppConfig) =>
  new Object({ ...defaultConfig, ...options });
...
export class ValorantXmppClient extends EventEmitter {
  ...
  getXmppInstance = async (): Promise<XmppClient> =>
    new Promise((resolve) => {
      const onReady = () => {
  ...
  get presence() {
    if (
      typeof this._presence === "undefined" ||
      !(this._presence instanceof PresenceBuilder)
    )
      this.presence = new PresenceBuilder().addKeystonePresence(
        new KeystonePresenceBuilder()
      );
  ...
  get reconnects() {
    this._reconnects = this._reconnects.filter(
      (date) => date + this._config.reconnectAttemptsTimeframe > Date.now()
    );
  ...
  async _mainLoop() {
    try {
      ...
      while (true) {
        const responses: Array<[string, any]> = Object.entries(
          await this._xmppInstance.readXml()
        );
        ...
        for (const response of responses) {
          ...
          if (Array.isArray(data))
            for (const xmlObj of data) this._handleData(type, xmlObj);
      ...
      if (this.reconnects.length > this._config.maxReconnectAttempts)
        return this.emit(
          "error",
          new Error(
            "There have been too many reconnects within specified timeframe."
          )
        );
  ...
  async login(options?: PasswordAuth | TokenAuth | CookieAuth) {
    ...
    // connect to xmpp
    this._xmppInstance = await new XmppClient().connect({
      port: 5223,
      host: `${this.tokenStorage.region.affinity}.chat.si.riotgames.com`,
    });

    // send declaration
    await this._xmppInstance.send(
      xmlDeclaration(this.tokenStorage.region).replace(/\s\s+/gi, "")
    );

    // read declaration and mechanism request
    if (
      typeof XmppClient.parseXml(await this._xmppInstance.read())[
        "stream:features"
      ] === "undefined"
    )
    ...
    // send mechanism
    await this._xmppInstance.sendXml(
      mechanism(this.tokenStorage.accessToken, this.tokenStorage.pasToken)
    );

    // check if auth was succesful
    const { success } = await this._xmppInstance.readXml();
    if (typeof success === "undefined") throw new Error("auth error");
  ...
  sendPresence = async () =>
    (await this.getXmppInstance()).send(
      XmppClient.buildXml(this.presence._toXmlObject())
    );
  ...
  sendFriendRequest = async (name: string, tagline: string) => {
    await (
      await this.getXmppInstance()
    ).sendXml(sendFriendRequest(name, tagline));
  };
  ...
  acceptAllFriendRequests = async () => {
    ...
    for (const friend of friends)
      await (
        await this.getXmppInstance()
      ).sendXml(sendFriendRequest(friend.name, friend.tagline));
  };
  ...
  removeAllSentRequests = async () => {
    ...
    for (const friend of friends)
      await (
        await this.getXmppInstance()
      ).sendXml(removeOutgoingFriendRequest(friend.jid.jid));
  };
...
export declare interface ValorantXmppClient {
  on<U extends keyof ValorantXmppClientEvents>(
    event: U,
    listener: ValorantXmppClientEvents[U]
  ): this;
  ...
  emit<U extends keyof ValorantXmppClientEvents>(
    event: U,
    ...args: Parameters<ValorantXmppClientEvents[U]>
  ): boolean;
}
